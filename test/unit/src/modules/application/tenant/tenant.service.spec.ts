...
import { mockCompanyEntity, mockCreateCompanyToEntityDto } from '../../../../mock/company.mock';
...
describe('Service: tenant', () => {
  ...
  describe('Exists', () => {
    it('should call RepositoryTenant.findOneBy with correct document', async () => {
        ...
        expect(tenantRepository.findOneBy).toHaveBeenCalledWith({document: 'any_document'});
    });

    it('should throws if RepositoryTenant.findOneBy throws', async () => {
        jest.spyOn(tenantRepository, 'findOneBy').mockImplementationOnce(async() => {
            throw new Error();
        });
  ...
  describe('ExistsIdentifier', () => {
    it('should call RepositoryTenant.findOneBy with correct indentifier', async () => {
        ...
        expect(tenantRepository.findOneBy).toHaveBeenCalledTimes(1);
        expect(tenantRepository.findOneBy).toHaveBeenCalledWith({companyIdentifier: 'any_identifier'});
    });
  ...
  describe('FindByUuid', () => {
    ...
    it('should throws if RepositoryTenant.findOne throws', async () => {
        jest.spyOn(tenantRepository, 'findOne').mockImplementationOnce(async() => {
            throw new Error();
        });
