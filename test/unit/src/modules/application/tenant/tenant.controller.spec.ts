...
describe('Controller: Tenant', () => {
    ...
    describe('CREATE', () => {
        ...
        it('should throws if TenantService.exists returns true', async () => {
            const data = mockCreateCompanyDto();
            jest.spyOn(tenantService, 'exists').mockReturnValueOnce(
                Promise.resolve(true),
            );
            const promise = sutTenantController.create(data);
            await expect(promise).rejects.toThrow(
                new Error('A empresa já está cadastrada!'),
            );
        });

        it('should throws if TenantService.exists', async () => {
            const data = mockCreateCompanyDto();
            jest.spyOn(tenantService, 'exists').mockImplementationOnce(
                async () => {
                    throw new Error();
                },
            );
        ...
        it('should call TenantService.existsIdentifier with correct identifier', async () => {
            ...
            expect(tenantService.existsIdentifier).toHaveBeenCalledWith(
        ...
        it('should throws if TenantService.existsIdentifier returns true', async () => {
            const data = mockCreateCompanyDto();
            jest.spyOn(tenantService, 'existsIdentifier').mockReturnValueOnce(
                Promise.resolve(true),
            );
            const promise = sutTenantController.create(data);
            await expect(promise).rejects.toThrow(
                new Error('A identificação já está cadastrada!'),
            );
        });
        ...
        it('should throws if TenantService.create throws', async () => {
            const data = mockCreateCompanyDto();
            jest.spyOn(tenantService, 'create').mockImplementationOnce(
                async () => {
                    throw new Error();
                },
            );
